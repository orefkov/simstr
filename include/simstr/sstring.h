...
struct simple_str : str_algs<K, simple_str<K>, simple_str<K>> {
    ...
    template<typename S> requires (std::is_same_v<std::remove_cvref_t<S>, std::string_view> && std::is_lvalue_reference_v<S>)
    constexpr simple_str(S&& s) noexcept : str(s.data()), len(s.length()){}
    ...
    bool is_same(const simple_str<K>& other) const noexcept {
...
struct simple_str_nt : simple_str<K> {
    ...
    explicit simple_str_nt(const K* p) noexcept {
...
inline constexpr size_t fnv_hash(const K* ptr, size_t l) {
    ...
    if (l <= maxLenForHash) {
        ...
        size_t delta = l / maxLenForHash, add = l % maxLenForHash, current = 0;
...
inline constexpr size_t fnv_hash(T&& value) {
    ...
    if constexpr (N - 1 <= maxLenForHash) {
        ...
        size_t delta = (N - 1) / maxLenForHash, add = (N - 1) % maxLenForHash, current = 0;
        for (size_t i = 0; i < maxLenForHash; i++) {
 ...
 * Хэширование выполняется алгоритмом FNV-1A, но из строки берётся не более maxLenForHash символов,
 * равномерно распределённых по строке.
