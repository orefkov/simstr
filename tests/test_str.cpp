...
TEST(CoreAsStr, StringData) {
    {
        lstringa<10> test = "dd";
        EXPECT_EQ(stra{test.data()}, "dd");
        static_assert(!std::is_const_v<std::remove_pointer_t<decltype(test.data())>>, "data() must be non const");
    }
    {
        const lstringa<10> test = "dd";
        EXPECT_EQ(stra{test.data()}, "dd");
        static_assert(std::is_const_v<std::remove_pointer_t<decltype(test.data())>>, "data() must be const");
    }
    {
        stringa test = "dd";
        EXPECT_EQ(stra{test.data()}, "dd");
