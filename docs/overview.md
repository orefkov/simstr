...
Все классы владеющих строк могут быть преобразованы в `simple_str_nt`, так как хранят строки с завершающим нулём.
...
Любая владеющая строка (sstring, lstring) может инициализироваться строковым выражением — она запрашивает у него длину,
...
Выдает пустую строку. Сокращённая запись — eea, eeu, eew, eeuu. Применяется обычно как пустая подчасть в `e_choice`, либо если формирование строки начинается с числа и строкового литерала:
```cpp
	str = eea + count + " times.";
```
...
## Заключение
На этом я пока заканчиваю краткий обзор строковой библиотеки `simstr`.
Пока нет подробного описания каждого из методов строковых объектов — это будет вторая большая часть.
В третьей части будут результаты бенчмарков.
Надеюсь, теперь стало немного более понятно, что и зачем в ней используется.
