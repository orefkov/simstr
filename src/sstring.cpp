...
SIMSTR_API size_t unicode_traits<u8s>::hashia(const u8s* src, size_t l) {
    ...
    if (l <= maxLenForHash) {
        ...
        for (size_t i = 0; i < maxLenForHash; i++) {
            h = (h ^ (uu8s)makeAsciiLower(src[current])) * fnv::prime;
            current += delta;
            if (add) {
                current++;
                add--;
...
SIMSTR_API size_t unicode_traits<u8s>::hashiu(const u8s* src, size_t l) {
    ...
    if (l <= maxLenForHash) {
        ...
        while (ptr < pEnd) {
            ...
            for (unsigned i = 0; i < delta && ptr < pEnd; i++) {
                readUtf8Symbol(ptr, pEnd);
            }
            if (add) {
                readUtf8Symbol(ptr, pEnd);
                add--;
...
SIMSTR_API size_t unicode_traits<u16s>::hashia(const u16s* src, size_t l) {
    ...
    if (l <= maxLenForHash) {
        ...
        size_t delta = l / maxLenForHash, add = l % maxLenForHash, current = 0;
        for (size_t i = 0; i < maxLenForHash; i++) {
            h = (h ^ makeAsciiLower(src[current])) * fnv::prime;
            current += delta;
            if (add) {
                current++;
                add--;
...
SIMSTR_API size_t unicode_traits<u16s>::hashiu(const u16s* src, size_t l) {
    ...
    if (l <= maxLenForHash) {
        ...
        while (src < pEnd) {
            ...
            for (unsigned i = 0; i < delta && src < pEnd; i++) {
                readUtf16Symbol(src, pEnd);
            }
            if (add) {
                readUtf16Symbol(src, pEnd);
                add--;
...
SIMSTR_API size_t unicode_traits<u32s>::hashiu(const u32s* src, size_t l) {
    ...
    if (l <= maxLenForHash) {
        ...
        size_t delta = l / maxLenForHash, add = l % maxLenForHash, current = 0;
        for (size_t i = 0; i < maxLenForHash; i++) {
            h = (h ^ makeFoldU(src[current])) * fnv::prime;
            current += delta;
            if (add) {
                current++;
                add--;
